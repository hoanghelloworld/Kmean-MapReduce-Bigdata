...
def main(src_img, dst_folder, k):
    ...
    img_name = splitext(basename(src_img))[0]

    # Create output file paths based on the image name
    points_path = join(dst_folder, f'{img_name}_points.txt')
    clusters_path = join(dst_folder, f'{img_name}_clusters.txt')
